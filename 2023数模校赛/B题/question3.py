...
t1 = time.time()
Mdl = OutputCodeClassifier(KNeighborsClassifier(n_neighbors=20, n_jobs=-1), n_jobs=-1)
Mdl.fit(train, label)
t2 = time.time()
...
t1 = time.time()
Mdl = OutputCodeClassifier(KNeighborsClassifier(n_neighbors=5, n_jobs=-1), n_jobs=-1)
Mdl.fit(train, label)
t2 = time.time()
print(f'Training time: {t2 - t1:.2f}s')
