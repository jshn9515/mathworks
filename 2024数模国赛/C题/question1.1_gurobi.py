...
# 农作物产量（与2023年相比较小值）
y = model.addMVar((seasons, crops, years), name='y')
# 农作物产量（原始产量）
...
for k in range(years - 3):
    # 平旱地梯田山坡地：单季种植
    model.addConstrs(
        (x[i, beans1, k : k + 3].sum() >= areas[i] for i in range(26)), name='R2'
    )
    # 普通大棚：第一季
    model.addConstrs(
        (x[i, beans2, k : k + 3].sum() >= areas[i] for i in range(26, 50)), name='R3'
    )
    # 智慧大棚：第一季 + 第二季
    idx1 = np.ix_(range(50, 54), beans2, range(k, k + 3))
    idx2 = np.ix_(range(78, 82), beans2, range(k, k + 3))
    total = x[idx1] + x[idx2]  # type: ignore
    model.addConstrs((total[i].sum() >= areas[-4 + i] for i in range(4)), name='R4')

for k in range(years):
    for j in range(crops):
        # 第一季：总产量取较小值
        model.addConstr(
            z[0, j, k] == (x[:54, j, k] * df4.values[:54, j]).sum(), name='T1'
        )
        model.addGenConstrMin(
            y[0, j, k].item(),
            [z[0, j, k].item()],
            constant=max_crop_season1[j],
            name='MIN1',
        )
        # 第二季：总产量取较小值
        model.addConstr(
            z[1, j, k] == (x[54:, j, k] * df4.values[54:, j]).sum(), name='T2'
        )
        model.addGenConstrMin(
            y[1, j, k].item(),
            [z[1, j, k].item()],
            constant=max_crop_season2[j],
            name='MIN2',
        )
...
with pd.ExcelWriter('问题1.1-结果.xlsx') as writer:
    for k in range(years):
