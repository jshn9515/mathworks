...
for k in range(years):
    for j in range(crops):
        # 第一季：总产量取较小值
        model.addConstr(z[0, j, k] == (x[:54, j, k] * df4.values[:54, j]).sum(), name='T1')
        model.addGenConstrMin(y[0, j, k].item(), [z[0, j, k].item()], constant=max_crop_season1[j], name='MIN1')
        # 第二季：总产量取较小值
        model.addConstr(z[1, j, k] == (x[54:, j, k] * df4.values[54:, j]).sum(), name='T2')
...
model.write('question1.1.mps')

if os.path.exists('question1.1.sol'):
    model.read('question1.1.sol')

model.optimize()
assert model.Status == gp.GRB.OPTIMAL
print(f'The profit is: {model.ObjVal:.4f}')
model.setParam('IgnoreNames', 0)
model.write('question1.1.json')
