...
for k in range(years - 3):
    ...
    model.addConstrs(
        (total[i].sum() >= areas[-4 + i] for i in range(total.shape[0])), name='R4'
    )
...
for k in range(years):
    for j in range(crops):
        # 第一季：总产量
        model.addConstr(
            z[0, j, k] == (x[:54, j, k] * df4.iloc[:54, j].to_numpy()).sum(), name='T1'
        )
        # 第一季：总产量取较小值
        ...
        # 第二季：总产量
        model.addConstr(
            z[1, j, k] == (x[54:, j, k] * df4.iloc[54:, j].to_numpy()).sum(), name='T2'
        )
        # 第二季：总产量取较小值
        model.addGenConstrMin(
            y_min[1, j, k].item(),
