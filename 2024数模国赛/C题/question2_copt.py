...
for k in range(years):
    ...
    # 普通大棚+智慧大棚：第一季
    model.addConstrs(x[34: 54, :15, k] == 0, nameprefix='C2')
    model.addConstrs(x[34: 54, 34:, k] == 0, nameprefix='C3')
    # 普通大棚：第二季
    model.addConstrs(x[62: 78, :37, k] == 0, nameprefix='C4')
    # 智慧大棚：第二季
    model.addConstrs(x[78:, :15, k] == 0, nameprefix='C5')
    model.addConstrs(x[78:, 34:, k] == 0, nameprefix='C6')
    # 土地种植面积限制
    model.addConstrs(x[:, :, k].sum(axis=1) <= df1['地块面积/亩'].to_numpy(), nameprefix='R1')
...
for k in range(years):
    for j in range(crops):
        # 第一季：总产量
        model.addConstrs(z[0, j, k] == (x[:54, j, k] * produce[:54, j, k]).sum(), nameprefix='T1')
        # 第一季：总产量取较小值
        ...
        # 第二季：总产量
        model.addConstrs(z[1, j, k] == (x[54:, j, k] * produce[54:, j, k]).sum(), nameprefix='T2')
        # 第二季：总产量取较小值
...
if os.path.exists('question2.sol'):
    model.read('question2.sol')
