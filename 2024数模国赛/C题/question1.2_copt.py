...
# 旱地不能种植某些作物
a = model.addConstrs(x[:26, 15:, :] == 0, nameprefix='C1')
# 普通大棚+智慧大棚：第一季
...
for k in range(years):
    for j in range(crops):
        # 第一季：总产量
        model.addConstr(z[0, j, k].item() == (x[:54, j, k] * df4.iloc[:54, j].to_numpy()).sum().item(), name='T1')
        # 第一季：总产量取较小值
        ...
        # 第二季：总产量
        model.addConstr(z[1, j, k].item() == (x[54:, j, k] * df4.iloc[54:, j].to_numpy()).sum().item(), name='T2')
        # 第二季：总产量取较小值
