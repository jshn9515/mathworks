...
df = pd.read_excel('附件/附件2(Attachment 2)2023-51MCM-Problem B.xlsx', index_col=0, parse_dates=True)
df.index = cast(pd.DatetimeIndex, df.index)

df['年份'] = df.index.year
df['季度'] = df.index.month // 3 + 1

df.insert(0, '发货-收货城市对', df['发货城市 (Delivering city)'] + '-' + df['收货城市 (Receiving city)'])
