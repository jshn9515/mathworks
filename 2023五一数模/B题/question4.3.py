...
def k_shortest_paths_length(G: nx.DiGraph, source: str, target: str, k: int) -> Generator[float]:
    paths = islice(nx.shortest_simple_paths(G, source, target, weight='固定成本 (Fixed cost)'), k)
    for path in paths:
        weight = 0
...
def calculate_price(base: int, cargo: int, cost: list[float]) -> float:
    ...
    if cargo % base == 0:
        count -= 1
    price = cost[count] * (1 + ((cargo - base * count) / base) ** 3) + sum(cost[:count]) * 2
    return price
...
df = pd.DataFrame(index=time_range, columns=['额定装货量-' + str(i) for i in sensitivity])
...
with pd.ExcelWriter('问题4-城市快递运输费用.xlsx', engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
    df.to_excel(writer, sheet_name='敏感性检验', index_label='日期', float_format='%.4f')
