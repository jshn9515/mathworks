...
== 化学成分的处理
成分数据是一类特殊的数据，其值恒为正值，当成分数据以百分比形式呈现时，各类比例累加和为1#super[@enwiki:1260922170]。从统计学角度看，成分数据的自由度为$n-1$，即每个样本数据因受比例累加和的限制导致其不具有相互独立性。从问题的实际意义看，成分数据的信息表示样本含量占总体的相对大小，而非比例成分的绝对数值。从数据特点看，成分数据具有高维、稀疏、右偏的特点，这表明数据处理时要进行必要的降维。

== 缺失异常值的处理
赛题中提到，成分数据的缺失值表示未检测到该成分，对于本数据集统一采用0.04进行填充，其合理性体现在两方面：

其一，根据维基百科#super[@enwiki:1292353507]，成分数据的缺失值或零值通常是由于检测仪器的下限值导致，例如对于化学成分占总体的含量小于某一阈值的化学成分，可能因为仪器的精确度而导致被视作不含有该成分，考察本数据集非零数据的最小值为0.04%，可认为GIA检测仪器的测量阈值为0.04%。
...
#figure(
    image("数据/问题1-风化前后化学成分含量箱线图.svg"),
...
=== 核密度定性分析
为进一步对玻璃化学成分进行定性分析，本文采用非参数核密度估计法。非参数核密度估计可完全基于数据驱动实现可靠的概率密度估计，是一种从数据样本本身出发，研究数据分布特征的方法#super[@enwiki:1289072338]。玻璃在化学成分上的占比数据离散，对统计数据进行极限近似，绘制非参数核密度估计图，以直观表示不同化学成分在玻璃表面有无风化上的统计特征。
...
#figure(
    image("数据/问题1-风化前后铅钡化学成分含量核密度图.svg"),
...
=== U检验的定量分析
曼-惠特尼U检验，又称“曼-惠特尼秩和检验”，是一种常用的非参数检验方法，用于比较两组相互独立样本是否来自相同分布的总体#super[@enwiki:1284635664]。该方法不依赖于样本服从正态分布的假设，适用于分布未知或难以满足参数检验前提条件的场景。其基本思想是基于样本数据的秩次而非原始观测值进行推断。
...
同理，得到高钾玻璃风化前后化学成分含量的曼-惠特尼U检验结果如@tbl:K2MnO4_U_test。
...
#show table.cell: set text(size: 10pt)
#let results = csv("数据/问题1-各化学成分的均值差.csv")
#figure(
    table(
        columns: 7,
        align: center + horizon,
...
将均值差运算后的数据进行中心对数比变换的逆变换，得到风化文物在未风化状态下的成分数据，还原结果如@tbl:BaO2_reduction 和@tbl:K2MnO4_reduction。

#show table.cell: set text(size: 6.5pt)
#let results = csv("数据/问题1-铅钡还原数据.csv")
#figure(
    table(
        columns: 15,
        align: center + horizon,
...
#let results = csv("数据/问题1-高钾还原数据.csv")
#figure(
    table(
        columns: 15,
        align: center + horizon,
...
#figure(
    image("数据/问题2-决策树重要性比较图.svg"),
    caption: [决策树分类重要性],
    placement: none,
) <fig:decision_tree_importance>

树类算法在机器学习中常用于因果效应的检验#super[@CausalTrees2018]。本文将附件中的样本数据按照46:21的比例划分训练集和测试集，并采用十折交叉验证，通过测试多种树类分类方法的准确率，以此确保该方法分类的合理性，从而根据树类算法寻找基于基本信息的分类重要特征。在树类算法中决策树算法适用于小样本机器学习。
...
#profiler.update(0)
+ *基于特征选择的数据降维*
14个不同的化学物质产生了14个不同的维度，而附件中的高钾玻璃仅有18个，数据产生了高维现象。以此进行分类规律的研究会因指标过少，造成过拟合现象，使得模型不具有可解释性#super[@enwiki:1286258242]。本文采取特征选择的方式对14维数据进行降维，筛选出重要程度较高的几个变量，对铅钡玻璃和高钾玻璃的分类规律展开研究。
...
支持向量机（SVM）是一种监督学习方法，常用于分类问题#super[@enwiki:1291923741]。SVM的核心思想是寻找一个最优超平面来最大化类间间隔，以实现良好的泛化能力。具体而言，设训练集 $x=[x_1,x_2,dots,x_N]$，标签 $y=[y_1,y_2,dots,y_N]$，SVM试图找到一个超平面：
...
#profiler.update(0)
+ *特征的选取*
根据中心对数比变换的结果，可知每一个数值均代表该种化学成分在该种文物下的相对重要性。而方差能反映一组数据的离散程度，在本问题中能反映化学成分的相对重要性，在不同文物之间的变化离散性。故一个化学成分的方差越大，亚类划分的意义就越明显#super[@enwiki:1291923741]。基于PCA的思想，本文选取方差的大小作为特征提取的依据，在铅钡玻璃和高钾玻璃中分别选择方差最大的两个化学成分。其计算公式如下：
...
层次聚类通过计算不同类别数据点间的相似度来创建一棵有层次的嵌套聚类树。其聚类主要根据的是数据与数据之间的“距离”判别其相似程度#super[@enwiki:1291783703]。相比于其他聚类算法，层次聚类使用“距离度量”为依据。从每一个玻璃文物出发，每一次通过合并最相似的聚类来形成上一层次中的聚类，当所有文物被聚为一类时，聚类算法停止。

#figure(
    image("图片/问题2-聚类示意图.png", width: 300pt),
    caption: [聚类算法原理],
    placement: none,
)
...
对于层次聚类的聚类层级划分，本文采用层次聚类树中每个链接的不一致系数进行确定。通过将每个链接的高度与其在层级结构中同一级别其他链接的平均高度进行比较，来计算每个链接的不一致系数。系数越大，表示该链接所连接的对象之间的差异越大。从具有最高不一致系数的节点开始，将该节点及其所有子节点归为一个簇。本文据此将铅钡玻璃和高钾玻璃均分为3类，聚类过程图如@fig:BaO2_clustering 和@fig:K2MnO4_clustering。

#figure(
    image("数据/问题2-铅钡聚类图.svg", width: 400pt),
    caption: [铅钡玻璃的聚类树],
    placement: none,
) <fig:BaO2_clustering>

#figure(
    image("数据/问题2-高钾聚类图.svg", width: 400pt),
    caption: [高钾玻璃的聚类树],
    placement: none,
) <fig:K2MnO4_clustering>
...
#figure(
    image("数据/问题2-铅钡SVM分类图.svg", width: 400pt),
    caption: [铅钡玻璃的分类器],
    placement: none,
...
#figure(
    image("数据/问题2-高钾SVM分类图.svg", width: 400pt),
    caption: [高钾玻璃的分类器],
    placement: none,
) <fig:K2MnO4_SVM_classifier>
...
由@fig:K2MnO4_SVM_classifier 分类器示意图可知，满足(1)的高钾玻璃属于“高钠富钾”型。不满足(1)时，若满足(2)，则属于“低钠富钾”型；若不满足(2)，则属于“低钠高钾”型。
...
#profiler.update(0)
+ *Pearson相关系数进行相关性分析*
由于风化的高钾玻璃中存在一些化学成分，其含量值未被检测到，因此本文对这些成分补0.04后进行剔除。通过估算玻璃文物样本化学成分的协方差和标准差，得到Pearson相关系数。Pearson相关系数可用于衡量两个变量间的线性关系#super[@enwiki:1290799627]，从而得到相关性图，并通过其值定量描述不同类型的玻璃在不同的风化情况下，与各化学成分含量之间的关联关系。
...
双样本K-S检验法（Kolmogorov-Smirnov检验法）是一种非参数检验方法，能够对不清楚数据分布特征的两组数据进行分布一致性检验，且对小样本数据同样具有良好的检验效果#super[@enwiki:1289549258]。其基本思想是：对两个样本大小分别为 $N_1$ 和 $N_2$ 的数据 $x_1$ 和 $x_2$，首先计算两样本的经验累积分布函数 $hat(F)_1(x_1)$ 和 $hat(F)_2(x_2)$。检验统计量定义为 $hat(F)_1(x_1)$ 与 $hat(F)_2(x_2)$ 之间的最大绝对差值，用以衡量两者的一致程度。
...
+ *皮尔逊相关系数法相关性分析*
分别对高钾玻璃和铅钡玻璃各化学成分之间作相关性分析，计算相关系数并绘制热力图，如@fig:BaO2_correlation_matrix、@fig:K2MnO4_correlation_matrix 所示。

由@fig:BaO2_correlation_matrix，氧化锶与二氧化硅、氧化锡和氧化钙3种化学成分之间的相关系数$|r|>0.7$，表现出高度相关性，这种强相关性可能反映了在材料成分配比或矿物结构中某种潜在的协同变化机制。相较之下，其余化学成分之间表现出中度或者低度相关性，说明它们之间的线性相关性较弱或中等，未表现出明显的协同趋势。由@fig:K2MnO4_correlation_matrix，所有化学成分之间表现出中度或者低度相关性，未发现明显的高度相关对。其中，氧化钾和氯化铅与其他13个化学成分之间的相关系数$|r|<0.4$，表明它们之间可能不存在稳定的线性依赖关系，也提示可能存在非线性相关性或更复杂的相互作用模式。

#figure(
    image("数据/问题4-铅钡相关系数矩阵.svg"),
    caption: [铅钡玻璃化学成分相关系数热力图],
    placement: none,
) <fig:BaO2_correlation_matrix>

#figure(
    image("数据/问题4-高钾相关系数矩阵.svg"),
    caption: [高钾玻璃化学成分相关系数热力图],
    placement: none,
) <fig:K2MnO4_correlation_matrix>
