...
df1 = pd.read_excel('附件/附件.xlsx', sheet_name='表单1', index_col=0)
...
df2 = pd.read_excel('附件/附件.xlsx', sheet_name='表单2', index_col=0)
...
df3 = pd.read_excel('附件/附件.xlsx', sheet_name='表单3', index_col=[0, 1])
...
df2['文物编号'] = df2.index
erode_idx = df2.index.str.contains('严重风化点', na=False)
not_erode_idx = df2.index.str.contains('未风化点', na=False)
df2.index = df2.index.astype(str)
df2.index = df2.index.str.extract(r'^(\d+)', expand=False)
df2.index = df2.index.astype(int)
df4 = df1.join(df2, how='right')
df4.loc[erode_idx, '表面风化'] = '风化'
df4.loc[not_erode_idx, '表面风化'] = '无风化'
