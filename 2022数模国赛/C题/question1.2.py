...
fig.savefig('数据/问题1-风化前后化学成分含量箱线图.svg', dpi=300)
...
fig.savefig('数据/问题1-风化前后铅钡化学成分含量核密度图.svg', dpi=300)
...
fig.savefig('数据/问题1-风化前后高钾化学成分含量核密度图.svg', dpi=300)
...
with pd.ExcelWriter(
    '数据/问题1-假设检验.xlsx', mode='a', engine='openpyxl', if_sheet_exists='replace'
) as writer:
