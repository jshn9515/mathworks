...
tbl = readtable('附件/附件-预处理后数据.xlsx', Sheet='表单1', TextType='string', ReadRowNames=true);
...
writetable(result, '数据/问题1-假设检验.xlsx', Sheet='卡方检验', ...
    WriteRowName=true, WriteMode='overwritesheet');

%% Question 1.2
clear; clc;

opts = detectImportOptions('附件/附件-预处理后数据.xlsx', Sheet='表单4', ...
    TextType='string', ReadRowNames=true);
opts = setvartype(opts, opts.VariableNames(1:4), 'categorical');
tbl = readtable('附件/附件-预处理后数据.xlsx', opts);
...
exportgraphics(fig, '数据/问题1-风化前后化学成分含量箱线图.svg', Padding='figure');
...
exportgraphics(fig, '数据/问题1-风化前后铅钡化学成分含量核密度图.svg', Padding='figure');
...
exportgraphics(fig, '数据/问题1-风化前后高钾化学成分含量核密度图.svg', Padding='figure');
...
writetable(result1, '数据/问题1-假设检验.xlsx', Sheet='铅钡玻璃秩和检验', ...
    WriteRowNames=true, WriteMode='overwritesheet');
writetable(result2, '数据/问题1-假设检验.xlsx', Sheet='高钾玻璃秩和检验', ...
    WriteRowNames=true, WriteMode='overwritesheet');
...
if TypstExport
    ...
    tbl5 = round(tbl5, 2);
    writetable(tbl5, '数据/问题1-各化学成分的均值差.csv', WriteRowNames=true, Encoding='GBK');
end
...
writetable(tbl1, '数据/问题1-铅钡高钾还原数据.xlsx', Sheet='铅钡-还原', ...
    WriteRowNames=true, WriteMode='overwritesheet');
writetable(tbl3, '数据/问题1-高钾高钾还原数据.xlsx', Sheet='铅钡-还原', ...
    WriteRowNames=true, WriteMode='overwritesheet');

if TypstExport
    writetable(tbl1, '数据/问题1-铅钡还原数据.csv', WriteRowNames=true, Encoding='GBK');
    writetable(tbl3, '数据/问题1-高钾还原数据.csv', WriteRowNames=true, Encoding='GBK');
end