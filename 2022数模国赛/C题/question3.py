...
df1 = pd.read_excel('附件/附件-预处理后数据.xlsx', sheet_name='表单4', index_col=0)
df2 = pd.read_excel('附件/附件-预处理后数据.xlsx', sheet_name='表单3', index_col=0)
...
X = df1[top2].select_dtypes(include='number')
y = df1['类型']
Mdl1 = SVC(kernel='linear')
Mdl1.fit(X, y)
score1 = Mdl1.score(X, y)
predict1 = Mdl1.predict(df2[top2])
print(f'阶段1：支持向量机模型准确率：{score1:.2f}')
...
print(f'铅钡玻璃使用{var_BaO2[0]}，{var_BaO2[1]}作为分类依据')
print(f'高钾玻璃使用{var_KMnO4[0]}，{var_KMnO4[1]}作为分类依据')
...
print(f'阶段2：铅钡模型准确率：{score21:.2f}')
...
print(f'阶段2：高钾模型准确率：{score22:.2f}')

print(f'铅钡玻璃分类结果：{predict21}')
print(f'高钾玻璃分类结果：{predict22}')
