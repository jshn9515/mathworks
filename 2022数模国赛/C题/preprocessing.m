...
opts1 = detectImportOptions('附件/附件.xlsx', Sheet='表单1', TextType='string', ...
...
tbl1 = readtable('附件/附件.xlsx', opts1);
...
opts2 = detectImportOptions('附件/附件.xlsx', Sheet='表单2', TextType='string', ...
    ReadRowNames=true, VariableNamingRule='preserve');
opts2.VariableNames = extractBetween(opts2.VariableNames, '(', ')');

tbl2 = readtable('附件/附件.xlsx', opts2);
...
opts3 = detectImportOptions('附件/附件.xlsx', Sheet='表单3', TextType='string', ...
    ReadRowNames=true, VariableNamingRule='preserve');
...
tbl3 = readtable('附件/附件.xlsx', opts3);
...
writetable(tbl1, '附件/附件-预处理后数据.xlsx', Sheet='表单1', ...
    WriteRowNames=true, WriteMode='overwritesheet');
writetable(tbl2, '附件/附件-预处理后数据.xlsx', Sheet='表单2', ...
    WriteRowNames=true, WriteMode='overwritesheet');
writetable(tbl3, '附件/附件-预处理后数据.xlsx', Sheet='表单3', ...
    WriteRowNames=true, WriteMode='overwritesheet');
writetable(tbl4, '附件/附件-预处理后数据.xlsx', Sheet='表单4', ...
    WriteRowNames=true, WriteMode='overwritesheet');