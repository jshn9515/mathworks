...
if TypstExport:
    ...
    df5.index.name = '化学元素'
    df5.to_csv(
        '数据/问题1-各化学成分的均值差.csv', float_format='%.2f', encoding='utf_8_sig'
    )
...
with pd.ExcelWriter('数据/问题1-铅钡高钾还原数据.xlsx') as writer:
...
if TypstExport:
    ...
    df1.to_csv('数据/问题1-铅钡还原数据.csv', float_format='%.2f', encoding='utf_8_sig')
    df3.to_csv('数据/问题1-高钾还原数据.csv', float_format='%.2f', encoding='utf_8_sig')
