...
df1 = pd.read_excel('附件/附件-预处理后数据.xlsx', sheet_name='表单1', index_col=0)
df2 = pd.read_excel('附件/附件-预处理后数据.xlsx', sheet_name='表单4', index_col=0)

P1 = pd.crosstab(index=df1['类型'], columns=df1['颜色'], margins=True, margins_name='总计')
P2 = pd.crosstab(index=df1['类型'], columns=df1['纹饰'], margins=True, margins_name='总计')
P3 = pd.crosstab(index=df1['类型'], columns=df1['表面风化'], margins=True, margins_name='总计')
...
fig = plt.figure(1)
ax = fig.add_axes((0.35, 0.1, 0.3, 0.8))
...
top2 = df_diff.index[:2].to_list()
X = df2[top2].select_dtypes(include='number')
y = df2['类型']
Mdl2 = SVC(kernel='linear')
Mdl2.fit(X, y)
score = Mdl2.score(X, y)
print(f'支持向量机模型准确率：{score:.2f}')
