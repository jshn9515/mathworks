...
fig.savefig('数据/问题2-铅钡聚类图.svg', dpi=300)
...
fig.savefig('数据/问题2-高钾聚类图.svg', dpi=300)

df4 = pd.Series(T1, index=df1.index, name='类别')
df5 = pd.Series(T2, index=df2.index, name='类别')

with pd.ExcelWriter('数据/问题2-铅钡高钾聚类数据.xlsx') as writer:
...
fig.savefig('数据/问题2-铅钡SVM分类图.svg', dpi=300)
...
ax.legend(*scatter.legend_elements(), loc='upper right', title='Classes')
fig.subplots_adjust(left=0.08, bottom=0.08, right=0.95, top=0.97)
fig.savefig('数据/问题2-高钾SVM分类图.svg', dpi=300)
