...
tbl1 = readtable('附件/附件-预处理后数据.xlsx', Sheet='表单1', TextType='string', ReadRowNames=true);
tbl2 = readtable('附件/附件-预处理后数据.xlsx', Sheet='表单4', TextType='string', ReadRowNames=true);
...
exportgraphics(fig, '数据/问题2-决策树重要性比较图.svg', Padding='figure');
...
tbl = readtable('附件/附件-预处理后数据.xlsx', Sheet='表单4', TextType='string', ReadRowNames=true);
...
set(h, LineWidth=1);
exportgraphics(fig, '数据/问题2-铅钡聚类图.svg', Padding='figure');

fig = figure(2);
h = dendrogram(L2, 0, Labels=tbl2.Sample, ClusterIndices=T2, ShowCut=true);
set(h, LineWidth=1);
exportgraphics(fig, '数据/问题2-高钾聚类图.svg', Padding='figure');
...
writetable(tbl4, '数据/问题2-铅钡高钾聚类数据.xlsx', Sheet='铅钡', ...
    WriteRowNames=true, WriteMode='overwritesheet');
writetable(tbl5, '数据/问题2-铅钡高钾聚类数据.xlsx', Sheet='高钾', ...
    WriteRowNames=true, WriteMode='overwritesheet');
...
set(gca, FontName='Microsoft YaHei', FontSize=12);
exportgraphics(fig, '数据/问题2-铅钡SVM分类图.svg', Padding='figure');
...
title(lgd, 'Classes', FontSize=12);
set(gca, FontName='Microsoft YaHei', FontSize=12);
exportgraphics(fig, '数据/问题2-高钾SVM分类图.svg', Padding='figure');